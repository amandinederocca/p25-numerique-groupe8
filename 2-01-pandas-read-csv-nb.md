...
# le code
df = pd.read_csv('data/titanic.csv')
type(df)#le type est un pd.dataframe
```
...
# pas besoin de 6 chiffres après la virgule dans la précision
pd.options.display.precision = 2

df.describe()#calculs les paramètres pour se décrire
```
...
```{code-cell} ipython3
df[['Age', 'Fare']].describe()#donne les paramètres qu'on veut retrouver dans la table
```
...
df.Age == df['Age']#false c les valeurs manquantes
```
...
```{code-cell} ipython3
# le code
type(df['Age'])#renvoie le type de la colonne qui est le meme pour tous les éléments
```
...
```{code-cell} ipython3
# la table après
# remarquez que 'PassengerId'(peut nous servir car permet de servir d'identifiant à chaque passager
# n'est plus présenté de la même manière
df = df.set_index('PassengerId')
df.tail(2)#au lieu de garder le numéro de la ligne on repère tout par passengerid
```
...
# votre code
import pandas as pd
tab=pd.read_csv('data/petit-titanic.csv')
tab.head(2)
pd.read_csv


#problème 1: la première ligne a été mise en index des colonnes
#problème 2:toutes les informations sont rassemblées en deux colonnes
#problème 3:on a une indexation en numéro de lignes
```
...
```{code-cell} ipython3
# votre code
tab = pd.read_csv('data/petit-titanic.csv', sep=';', header=None, names=['PassengerId', 'Survived', 'Pclass', 'Name', 'Sex', 'Age', 'SibSp',
          'Parch', 'Ticket', 'Fare', 'Cabin', 'Embarked'])
tab.set_index('PassengerId', inplace=True)
tab.head()
```
...
```{code-cell} ipython3
# votre code
tab = pd.read_csv('data/petit-titanic.csv', sep=';', header=None, names=['Identifiant', 'Survécu', 'Pclass','Nom', 'Genre', 'Age', 'SibSp', 'Parch','Ticket', 'Tarif', 'Cabine', 'Embarquement'])
tab.head(2)
```
...
```{code-cell} ipython3
# votre code
tab.set_index('Identifiant', inplace=True)
tab.head()
```
