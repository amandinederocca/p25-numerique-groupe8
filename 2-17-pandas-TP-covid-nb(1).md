...
```{code-cell} ipython3
# votre code
france= by_country['France']#données de la France
dffrance = pd.DataFrame(france)#création de la dataframe
dffrance.head(4)
```
...
```{code-cell} ipython3
# les colonnes de cette dataframe sont les critères relevés lors des mesures
#une colonne correspond à une mesure:590
dffrance.value_counts()
```
...
```{code-cell} ipython3
# votre code
Italie= by_country['Italy']#données de la France
dfitaly = pd.DataFrame(Italie)#création de la dataframe
dfitaly['country']='Italy'
dfitaly.head(4)
dffranceitaly=pd.concat([dffrance, dfitaly])
dffranceitaly.value_counts()#on a bien rattaché les deux df avec maintenant 1180 mesures
```
...
for i in by_country.keys():
    pays= by_country[i]#données de la France
    dfpays = pd.DataFrame(pays)#création de la dataframe
    dfpays['country']=i
    dftot=pd.concat([dftot, dfpays])
...
```{code-cell} ipython3
# votre code
clean_df=dftot.set_index(['country','date'])
```
...
```{code-cell} ipython3
# votre code
mesures=clean_df.loc[('France','2021-01-01')]
print(mesures)
```
...
```{code-cell} ipython3
# votre code
italie=clean_df.loc['Italy']
franceitalie=pd.concat([france, italie])
```
...
```{code-cell} ipython3
# votre code
germany=clean_df.loc['Germany']
df3=pd.concat([franceitalie, germany])
df3.value_counts()
```
...
def extract(pays, mesures, deb, fin):
    df = dftot[dftot['country'].isin(pays)].copy()#je récupère les mesures sur les pays qui m'intéressent
    df['date'] = pd.to_datetime(df['date'])#en reprenant les etapes des questions précédentes je convertis les dates et récupère la plage de temps qui m'intéresse
    df = df[(df['date'] >= deb) & (df['date'] <= fin)]
    df = df[['date', 'country'] + mesures]#on sélectionne les colonnes sur lesquelles on veut des info: il nous faut absolument les dates et les pays car on classent les données par ces critères
    df = df.pivot(index='date', columns='country', values=mesures)
    df.columns = [f"{pays}{mesure}" for mesure, pays in df.columns]
...
# votre code
df=extract(pays=['France', 'Italy'], mesures=['deaths', 'confirmed', 'recovered'], deb='2020-01', fin='2021-09')#ne pas oublier de mettre les chaines de caractère entre guillemets
df.head(5)
...
```{code-cell} ipython3
# votre code
df.plot(figsize=(12,6), title="COVID")
plt.grid()
plt.show()
```
...
```{code-cell} ipython3
df5=extract(pays=['France', 'Italy'], mesures=['deaths', 'confirmed', 'recovered'], deb='2020-01', fin='2021-09')
pays=[France, Italy, Germany]
for country in pays:
    df5[f'{country}mortalité'] = df5[f'{country}deaths'] / df5[f'{country}confirmed']
```

```{code-cell} ipython3
df5.plot(figsize=(12,6), title="taux de mortalité")
plt.grid()
plt.show()
```

```{code-cell} ipython3
# je vais tracer le taux de mortalité covid par pays
df['mortalité']=df['deaths']/df['confirmed']
df.head(6)
```
