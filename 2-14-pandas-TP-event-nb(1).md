...
```{code-cell} ipython3
# your code
ok=events['end']>events['beg']
ok.all()
```

```{code-cell} ipython3
events.set_index('beg')
events.head(2)
```
...
#on trie par pays et par évènements chronologiques
eventstriés = events.sort_values(by=['country', 'beg'])
eventstriés['end:avant'] = eventstriés.groupby('country')['end'].shift(1)
eventstriés['overlap'] = eventstriés['beg'] < eventstriés['end:avant']#est-ce que le debut de la ligne est avant la fin de celle d'avant?
overlappingevents = eventstriés[eventstriés['overlap']]
...
overallreservationtime = events['duration'].sum()
overalltimespan = events['end'].max() - events['beg'].min()
usageratio = overallreservationtime / overalltimespan

print(overallreservationtime)
print(overalltimespan)
print(usageratio)
```
...
```{code-cell} ipython3
# resample
eventsindexés = events.set_index('beg')
durationmonth = eventsindexés['duration'].resample('M').sum() # month 'M', week 'W', year 'Y'
durationmonth.plot(kind='bar')
plt.show()
...
```{code-cell} ipython3
#to period
events['periodmonth'] = events['beg'].dt.to_period('M')
durationmonth2 = events.groupby('periodmonth')['duration'].sum()#je regroupe par périodes et somme les durées sur les mois
durationmonth2.plot(kind='bar')
plt.show()
```
...
```{code-cell} ipython3
durationweek = eventsindexés['duration'].resample('W').sum()
ax=durationweek.plot(kind='bar', figsize=(14,6), title="Total reservation duration per week (en s)")

plt.ylabel("Duration (s)")
plt.xlabel("Week")
num_bars = len(durationweek)
tickpositions = list(range(0, num_bars, 10))#on affiche qu'une barre toutes les 10 semaines pour etre plus lisible
ticklabels = [durationweek.index[i].strftime('%Y-%m-%d') for i in tickpositions]
...
```{code-cell} ipython3
# ma fonction n'a pas l'air de fonctionner...
import math

def converttimedeltaintohours(timedelta: pd.Timedelta) -> int:
    totalsec = timedelta.total_seconds()
    hours = math.ceil(totalsec / 3600)#on divise par 3600 et arrondi au dessus car toute heure commencée est terminée
    return hours
```
...
```{code-cell} ipython3
# your code
durationhours = durationweek.map(converttimedeltaintohours)#on applique la fonction pour convertir en heures
ax=durationhours.plot(kind='bar',figsize=(14,6),title="Total reservation ruration per week (hours)")

plt.ylabel("Duration (hours)")
plt.xlabel("Week")

num_bars = len(durationhours)
tickpositions = list(range(0, num_bars, 10))
ticklabels = [durationhours.index[i].strftime('%Y-%m-%d') for i in tickpositions]
...
events = events.merge(
    ...
    on='country',
    how='left'
)#on créer une colonne région dans events
...
weeklyduration = eventsindexés.groupby('region')['duration_hours'].resample('W').sum().unstack(0)#On rassemble par semaine et région
ax=weeklyduration.plot(kind='bar', figsize=(14,6), title="Weekly reservation duration by region (hours)")

plt.ylabel("Duration (hours)")
plt.xlabel("Week")

# Optional: improve X-axis ticks
num_bars = len(weeklyduration)
tickpositions = list(range(0, num_bars, 10))
ticklabels = [weeklyduration.index[i].strftime('%Y-%m-%d') for i in tickpositions]
ax.set_xticks(tickpositions)
ax.set_xticklabels(ticklabels)
